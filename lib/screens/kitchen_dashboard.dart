...
class _KitchenDashboardState extends State<KitchenDashboard> {
  ...
  // ✅ OPTIMIZED: Merge orders with parallel processing
  Future<void> mergeOrdersWithAlertState(
    ...
    // Collect orders that need to be auto-confirmed
    ...
    // Process reservations that are ready
    ...
    // ✅ OPTIMIZATION 1: Update status in PARALLEL (non-blocking)
    if (ordersToConfirm.isNotEmpty) {
      // Fire and forget - don't await here
      confirmOrdersInBackground(ordersToConfirm);

      // Immediately add to preparing for printing
    ...
    // ✅ OPTIMIZATION 2: PRINT IMMEDIATELY (don't wait for API)
    if (!isInitialLoad) {
      processPrintQueue(allPreparing); // Fire and forget
      processNewReservations(newReservations);
    } else {
      // Initial load: Track all existing items
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 4),
                                  Text(
                                    order.updatedAtWIB != null ? _formatTime(order.updatedAtWIB!) : '-',
                                    style: TextStyle(
                        ...
                        // Status indicator
                        if (isSelected) Icon(Icons.chevron_right, color: brandColor, size: 24),
                      ],
                    ),
                  ),
                ),
